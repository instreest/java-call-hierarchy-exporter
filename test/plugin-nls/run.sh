...
# コメント・文字列・文字リテラルを<b>頭から順に</b>読み分けて、文字列の中の仮名・漢字を探す。
# 1件でも残っていると、その行だけ英語の利用者に日本語が出る。
#
# コメントを先に消してから文字列を探す作りにはしない。文字列の中に // や /* があると
# （URL や正規表現）そこから先を消してしまい、<b>検査が黙って何も見なくなる</b>。
# 実際その作りだったとき、server/JdkDownload.java の日本語 10 件を素通ししていた。
# 順に読む（この並びの最初に当たったものを1つの塊として食べる）ことでその穴が無くなる。
#
# perl には -CSD が要る。これが無いと元のバイト列のまま扱われ、\x{3040}- の範囲が
# 1文字も当たらない（やはり検査が何も見なくなる）
LEFTOVER=$(find "$SRC" -name '*.java' -print0 | xargs -0 perl -CSD -0777 -ne '
    while (m{(/\*.*?\*/)|(//[^\n]*)|("(?:[^"\\]|\\.)*")|(\x27(?:[^\x27\\]|\\.)*\x27)|.}gs) {
        my $lit = $3;
