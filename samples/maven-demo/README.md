...
greeter の jar と POM をローカルリポジトリ（テストでは `library.repositories` で `samples/localrepo` を指定）から探し、
...
テストの `project.root` は `samples`（このフォルダの親）で、`pom.xml` の無い `project.root` から
ソースフォルダの上位を探す経路を通します。`<repositories>` は Maven でこのプロジェクトを実際にビルドする人の
ためのもので、ツールは見ません。
