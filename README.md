...
条件分岐の打ち切りで階層から消えた部分木は**ここでしか見えない**ので、
打ち切りが妥当かを確かめるときはこの2列で絞り込んでください。

| `absentCause` | 意味 |
|---|---|
| `条件分岐で打ち切った先` | [条件分岐による打ち切り](#条件分岐による打ち切り呼ばれない経路の判定)で階層から消えた。打ち切った呼び出し先から宣言上のエッジを辿って求めた範囲（別の経路で1行でも出たメソッドは含まない） |
| `除外パッケージ` | `exclude.packages` で除外された |
| `CHA候補のため未展開` | 実装を1つに絞れず、候補として行にはなるがその先へ降りなかった |
| `循環のため未展開` | 経路上で既に呼んでいるメソッドへ戻る辺だった |
| `上流が未出力` | そこへ至る呼び出し自体が出ていない（`max.depth` / `max.rows` の先、起点から辿り着かない、デッドコード） |
...
呼び出し階層CSVから丸ごと消えます。消えたメソッドは
[`methods.csv`](#methodscsv--ソース上の全メソッドとその呼び出し状況) の
`inHierarchy=0` / `absentCause=条件分岐で打ち切った先` で一覧できます
