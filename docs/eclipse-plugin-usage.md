...
2. 「Call Hierarchy Exporter」コンソールにも同じものが出る。**解析を始めると自動で前面に出る**ので、
   放っておいても様子が見える。子プロセスの標準エラー（JVM の警告・GC・`OutOfMemoryError` の
   スタックトレース）は `[stderr]` を付けて**赤**で出る。解析の失敗も赤で出る
...
| 解析が失敗する | 「Call Hierarchy Exporter」コンソール（解析を始めると自動で開く）に子プロセスの出力がそのまま出る。赤い行が子プロセスの標準エラーと失敗の理由（ログファイルにも残る）。設定ファイルの誤り（`project.root` など）が多い |
