...
2. バナーの［解析する］。メソッドを決める前に、解析だけ先に済ませておける
...
| 向きを変える | ツールバーの［呼び出し元を表示］／［呼び出し先を表示］。いまどちらを見ているかは、押されているほうで分かる |
...
|---|---|---|
| 解析キャッシュ | `<状態フォルダ>/cache` | この下の `.cache/<プロジェクト名>_<識別子>/` に `analysis-cache.tsv` と `dataflow-cache.tsv`。消しても次の解析で作り直す（時間はかかる） |
| 解析ログ | `<状態フォルダ>/log` | `analysis-<日時>.log`。新しいものから 10 世代 |
| CSV の出力 | `<状態フォルダ>/output` | ［CSV出力］の初期フォルダ。保存先はそのつど選べる |
