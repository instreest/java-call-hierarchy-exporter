...
| **解析結果を捨てる** | ツールバーの 🗑［解析結果をクリア］。確認のうえで捨て、解析前の画面に戻る。ディスクのキャッシュは残るので、次の解析は速く終わる |
...
結果が消えるのは次の 3 つだけである。
［解析結果をクリア］／［再解析］／解析プロセスの設定（JDK・JDT・VM 引数・キャッシュの置き場所）を
変えて［OK］。理由は [eclipse-plugin-ui-simplify-qa.md](eclipse-plugin-ui-simplify-qa.md) の Q1。
...
| 使われないときに終了する | **0（終了しない）** | 解析プロセスは結果をメモリに持って常駐する。分数を入れるとそのぶん放置で終了し、**保持している解析結果も消える**。メモリを空けたいときだけ使う |
