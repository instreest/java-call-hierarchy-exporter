...
| 進捗と作業ログをファイルにも残す | 残す | ワークスペースの中（`.metadata/.plugins/io.github.instreest.jche.eclipse/log/analysis-<日時>.log`）に、新しいものから10世代。［ログフォルダを開く］でそこを開ける |

### 解析が遅い・返ってこないとき

1. ［ログフォルダを開く］で最新の `analysis-*.log` を見る。解析の開始・進捗（10秒おき）・
