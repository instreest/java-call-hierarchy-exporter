...
| [micro-execution-qa.md](micro-execution-qa.md) | — | 静的解析で解けない断片（文字列の計算・jar からの呼び戻し）だけを実行して補う案（**不採用**）。「断片実行」と呼ぶ理由、単体テスト・実行トレース・モニタリングとの線引き、今のプラグインとの比較、対象のコードを実行する・関数の切り出しを求めるので採らない判断、代わりに解析器が文字列の計算を評価する方向（未実装） |
