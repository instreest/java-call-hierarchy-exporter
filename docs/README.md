...
| [jjug-order-export-sample-qa.md](jjug-order-export-sample-qa.md) | — | JJUG 発表用サンプル `samples/jjug-order-export`（Spring Boot + Doma）。呼び出し元で実装が変わるインターフェース呼び出しの題材の選び方、デファクトの書き方（`@Component` とコンストラクタ注入）では経路ごとに絞れず CHA になること・`new` で渡すと確定すること、回帰テストに入れない判断 |
