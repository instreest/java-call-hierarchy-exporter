...
| [spring-di-qa.md](spring-di-qa.md) | #66 | Spring の Bean 定義（`@Autowired` / `@Qualifier` / `@Bean`）で候補を絞る。絞るのはレシーバが注入点（注入の注釈の付いたフィールド・メソッドの引数、Bean のコンストラクタの引数と static でないフィールド）のときだけで、経路で具象型が分かればそちらを採る（PR #167 の見直しで改めた） |
