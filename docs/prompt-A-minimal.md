...
Javaプログラムを作ってください。実装の細部は任せますが、「目的」と
...
- **外部jarからの被参照**（`external.library.folders` 指定時）: `caller` = 参照している側の
  クラス名、`root` = jar名（FatJar の中の jar なら `外側.jar!/BOOT-INF/lib/中.jar` のように
  jar URL と同じ `!/` 区切りで場所まで）、末尾に `被参照:EXACT`（そのクラスで宣言されている
  メソッド。合成した暗黙のデフォルトコンストラクタを含む）/ `被参照:INHERITED`（親から継承した
  メソッド。宣言している**最も近い親**（親クラスの連鎖を先に、次にインターフェース）のメソッドと
  して出す）/ `被参照:IMPLICIT_CTOR`（引数なしコンストラクタへの参照で、ソース上に一致する宣言が
  無いもの。版違いの可能性が高いが生成箇所として有用なので残す）

行順は環境（OS・ファイルシステム・キャッシュの状態）に依存させない。起点はソースの並び
（ソースフォルダの指定順 → 完全修飾クラス名順 → 宣言行順）、起点からの展開はソース上の
呼び出し順（深さ優先）、複数候補は宣言型自身 → 下位型（完全修飾クラス名順）、末尾の
`(型解決失敗)` の行はソースフォルダ順 → ファイルの相対パス順 → 呼び出し順、被参照の行は
jar のパス順 → jar の中のクラスの順。ファイルの並びの鍵は `/` 区切りの相対パス文字列の比較
（`Path#compareTo` は OS で挙動が違う）。
...
（中身と関係なく変わるので、当てにすると「中身が同じなのに解析し直す」と
「中身が違うのに再利用する」の両方が起きる）。**準拠レベル（`source.level`）が
変わったらキャッシュ全体を破棄する**（同じソースでも解析結果が変わるため）。
さらに、そのファイル自身が変わっていなくても、**参照している型を宣言する
...
パッケージ）と、ファイルごとに JDT が報告したエラー数を残す。jar の指紋は、末尾の目次
（セントラルディレクトリ）の「エントリ名・サイズ・CRC」から作る（jar 本体を読まないので速い）。次回の実行で jar が追加・
差し替え・削除されていれば、その jar のパッケージの型を参照しているファイルと、前回型解決に
失敗していたファイルだけを解析し直す（「型解決できなかった呼び出しが N 件」と出たとき、
jar を足せばキャッシュを消さずに次の実行で反映される）。実行する JDK が変わったときは
キャッシュ全体を作り直す（JDK の版も解決結果に影響する暗黙の依存）。

パースは 100 ファイルずつまとめて行う（1ファイルずつでは規模に対して超線形に遅くなる）。

### 5.2 抽出する呼び出し

メソッド呼び出し、`super.m()`、`new`（コンストラクタ）、`this(...)`/`super(...)`（書かれて
いない暗黙の `super()` は拾わない）、メソッド参照（`obj::m` / `Type::m` / `super::m` / `Type::new`）。
...
- `exclude.packages` に一致するノードは行にしないが、**その先は親に繋ぎ直して辿り続ける**
  （`java.**` を除外しても、その先の自分のコードは出る）。読み飛ばし中の除外メソッドも循環判定の
  祖先に含め、読み飛ばしの入れ子にも深さ上限を掛ける（除外パッケージ内の相互再帰で
  スタックオーバーフローにならないため）
...
他チームのjarの中のclassファイルを自前で読み（外部ライブラリ不要）、自分のメソッドを
参照している箇所を列挙する。定数プールで参照先を、各メソッドの命令列（Code 属性）の
`invoke*` 命令と LineNumberTable で「どのメソッドの何行目から」を求める。ラムダ・メソッド参照
（`invokedynamic`）は BootstrapMethods 属性の MethodHandle 引数を辿る。行番号情報の無い
class（`-g:none`）はメソッド名だけ出し、行は `(Unknown Source)` にする。
...
**FatJar（jar の中の jar）**もそのまま指定できる。Spring Boot の実行可能 jar
（`BOOT-INF/classes/` と `BOOT-INF/lib/*.jar`）、war（`WEB-INF/classes/` と `WEB-INF/lib/*.jar`）、
ear（中に war や jar）を、中の jar を取り出さずにストリームで開いて何段でも辿る（安全策として
8 段まで）。同じ jar が複数の FatJar に入っていれば、それぞれの FatJar の行として別々に出す
（「どの配布物に影響するか」が答えなので、まとめない）。FatJar の中の自プロジェクト jar も
同じく読み飛ばす。
...
- 自己再帰・相互再帰で無限ループせず `[CYCLE]` が付く。除外パッケージの中だけで
  相互再帰していてもスタックオーバーフローにならない
- `caller` 列をEclipseの「Javaスタック・トレース・コンソール」に貼るとソースへジャンプできる
  （内部クラスは `Outer$Inner`）
- 依存 jar を足して再実行すると、キャッシュを消さなくても型解決失敗が減り、影響するファイル
  だけが解析し直された旨がログに出る
- 同じソースを Linux と Windows で解析して、両CSVが行順まで一致する
