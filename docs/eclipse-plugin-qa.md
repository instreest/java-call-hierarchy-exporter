...
> **いまは違う点**: ここに出てくる「設定ファイル（`*.properties`）を右クリック →
> 呼び出し階層をCSVに出力」は**廃止した**。同じことはコマンドライン版でできて、
> プラグインは別プロセスをもう 1 つ起こす経路を二重に抱えていたためである
> （[eclipse-plugin-ui-simplify-qa.md](eclipse-plugin-ui-simplify-qa.md) の Q10）。
> いまのプラグインの入口は呼び出し元階層ビューで、CSV はその右クリックから書き出す。
