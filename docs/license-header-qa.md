...
- ライセンス全文は `LICENSE` に一本化し、`NOTICE` に著作権表示をまとめる。README に「ライセンス」節を足す
...
Apache-2.0 が NOTICE を必須にするのは、既に NOTICE を持つ成果物を再配布する場合だけで、
このリポジトリに義務はない。それでも置いたのは、ファイル毎のヘッダを 1 行に削った結果、
「著作権者は誰か」をまとめて書いた場所が LICENSE 側になくなるため。1 行ヘッダと
LICENSE 全文の間を埋める位置づけ。
