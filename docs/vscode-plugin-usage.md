...
1. アクティビティバーの「影響調査」を開き、［解析する］を押す
   ...
   進捗は右下の通知に出る。中止できる
2. 解析が終わったら、エディタで**メソッドの中**にカーソルを置き、右クリック →「影響調査: 呼び出し元を表示」
...
メソッドの外（フィールド宣言やクラスの末尾）にカーソルがあるときは、直前のメソッドを返さずに
「メソッドの中にカーソルを置いてください」と出る（宣言の範囲を持っているため。[method-decl-range-qa.md](method-decl-range-qa.md)）。

## 表示言語

画面・通知・ログの文言は**英語が既定**で、VSCode の表示言語が日本語のときだけ日本語になります
（`vscode.env.language` を見ます。言語パックを入れ替えればそれに追従します）。

解析の子プロセスにも同じ言語を渡すので、**拡張の画面と解析のログが必ず同じ言語**になります。
出力する CSV の中身だけは言語で変わりません（常に英語。[nls-qa.md](nls-qa.md) の Q6）。
