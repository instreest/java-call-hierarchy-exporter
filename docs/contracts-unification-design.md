...
> **実装状況**（2026-09-19）: **§9 の段1・段2 まで完了**。`=>` の振り分けと C-1（宣言型）・
> C-2（型#メソッド名）・C-3（ファクトリ＋キー）が動く（`jche.graph.TypeContracts`。ラベルは
> `[RESOLVED:CONTRACT]`）。C-3 は設計どおり**フェーズAの証拠採取なしで**、dataflow キャッシュに
> 載っている実引数から引いている。`test/regression` の plugin ケースで、同梱の拡張
> （`FactoryKeyCollector` ＋ `TypeMappingProvider`）と**由来ラベル以外はまったく同じ出力**になることを
> 検査している。残りは §10 の 4（列挙定数のキー）と、段3（出力から設定へのひな形）。
...
**4. 列挙定数のキー（`get(Kind.USER)`）。** 値グラフでは `V:jp.co.app.Kind.USER`（`Origin.CONST` に
型 FQN ＋ 定数名）として載っていて、文字列リテラル（`Origin.LITERAL`）とは種別が違う。
`literalValueOf` も `LITERAL` しか返さないので、いまは当たらない。入れるなら
「引用符なしの左辺は列挙定数の FQN」と読む形になるが、引用符の有無で意味が変わる書式は
間違えやすい。**キーが列挙型のファクトリを実際に見てから決める**ことにして、いまは
引用符なしの行を「読めない行」として助言つきで弾いている。
