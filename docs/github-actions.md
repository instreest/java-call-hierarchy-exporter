...
その場合、アクションは `run.log` の依存 jar の警告（`[WARN] Dependency jars:`、日本語表示なら `[WARN] 依存jar:`）を GitHub の warning アノテーションとジョブサマリに出すので、
...
| `artifact-retention-days` | （空欄。リポジトリの既定＝通常 90 日） | アーティファクトの保持日数。プライベートリポジトリでは短くする（[保存量と保持期間](#保存量と保持期間)） |
...
設定ファイルごとに別の出力フォルダができます（[cli.md の「複数の設定ファイルをまとめて処理する」](cli.md#複数の設定ファイルをまとめて処理する)）。
...
1 回の実行で残るものは 3 つです。**課金の対象になるのはアーティファクトだけ**で、キャッシュと、
パブリックリポジトリなら実行時間は無料です。

| 残るもの | 大きさ（このリポジトリ自身の解析。ソース 131 ファイル・2.4 万行） | 増え方 |
| --- | --- | --- |
| アーティファクト | **zip 約 3.3MB**（5 ファイル。`call-hierarchy.csv` は 41 万行） | 実行ごとに 1 つ。保持日数が過ぎると消える |
| JBang と JDT のキャッシュ（`cache`） | 約 51MB | ツール本体が変わるまで 1 エントリのまま。増えない |
| AST 解析キャッシュ（`analysis-cache`） | 数 MB（数万ファイル規模なら数百 MB） | **実行ごとに 1 エントリ増える**（同じキーへ保存し直せないため） |

- **アーティファクト**はアカウントのストレージを使います。パブリックリポジトリでは無料ですが、
  プライベートリポジトリではプランごとの無料枠（GitHub Free なら 500MB）から引かれ、超えたぶんは
  従量課金になります（[GitHub Actions の課金](https://docs.github.com/ja/billing/managing-billing-for-github-actions/about-billing-for-github-actions)）。
  課金は「容量 × 置いてある期間」で効くので、**保持日数を短くするとそのぶん減ります**
- **キャッシュ**は課金の対象外ですが、リポジトリごとに 10 GB の枠があり、超えると古いものから消えます
  （7 日間使われないエントリも消えます）。この枠は `actions/setup-java` の Maven キャッシュなど
  **そのリポジトリのほかのキャッシュと共通**なので、解析キャッシュが大きいリポジトリでは、
  それらを押し出してしまうことがあります。気になる場合は `analysis-cache: 'false'` で切れます
- **実行時間**はパブリックリポジトリでは無料です。プライベートリポジトリでは課金され、ランナーで
  係数が変わります（Linux 1 倍 / Windows 2 倍 / macOS 10 倍）。解析結果はランナーによらず同じなので、
  **Linux ランナーで動かすことを勧めます**

### プライベートリポジトリで使うとき

`artifact-retention-days` は空欄だとリポジトリの既定（通常 90 日）になります。
**プライベートリポジトリで繰り返し動かす場合は、7 日程度にしておくことを勧めます。**

```yaml
      - uses: instreest/java-call-hierarchy-exporter@main
        with:
          source-folders: src/main/java
          artifact-retention-days: 7     # 既定はリポジトリの設定（通常 90 日）
```

1 回 3.3MB を 1 日 1 回作った場合、積み上がる量は保持日数にそのまま比例します。
...
業務システム規模で**全体モード**（`entry-packages` 空欄）を動かすと、これより 1 桁から 2 桁
大きくなります。行数はソースの行数におおよそ比例し（このリポジトリでは**ソース 1 行あたり約 17 行**）、
`call-hierarchy` 列に経路の全体が入るので、深いほど 1 行が長くなります。目安は
**ソース 1 万行あたり zip で 1〜2MB** で、呼び出しの枝分かれの多さで上下します。
`max.rows` の既定は 500 万行なので、ソースが 30 万行を超えるあたりで上限に達して打ち切られます
（打ち切ったことはログに警告が出ます）。

減らすときは、上から順に効きます。

1. **保持日数を短くする**（`artifact-retention-days: 7`）… 出力の中身は変えずに、日数のぶんだけ減ります
2. **起点を絞る**（`entry-packages`）… 全体モードをやめ、調べたい画面や API の入口だけを起点にします。
   行数が最も大きく減ります
3. **出力を減らす**（`extra-config` に `max.depth` を書いて浅くする、`exclude-packages` に
   フレームワークのパッケージを足す）
4. **動かす回数を減らす**（手動実行 `workflow_dispatch` や定期実行 `schedule` にして、push のたびには
   動かさない。自動実行を足すときは[セキュリティ上の注意](#セキュリティ上の注意)を先に読んでください）

CSV を後続のステップで読むだけでアーティファクトが要らない場合は、`upload-artifact: 'false'` で
アップロードそのものを切れます。
...
- **キャッシュ** … `cache: true` のとき、JBang 本体（`~/.jbang`）と JDT の jar（`~/.m2/repository/org/eclipse`）を
  ...
  チェックアウト時刻にしますが、同一性の判定はパスとサイズと内容のハッシュだけで行い更新時刻は見ないので、
  中身が同じなら再利用されます。キャッシュは実行ごとに新しいエントリとして保存され（`jche-analysis-<OS>-…`）、
  復元は「同じ設定ファイルの最新」→「同じ OS の最新」の順に前方一致で探します。古いエントリは GitHub が
  容量（リポジトリごとに 10 GB。そのリポジトリのほかのキャッシュと共通の枠です）と
  期限（7 日間使われないもの）で消します（[保存量と保持期間](#保存量と保持期間)）。
...
- **保存量** … 1 回の実行でアーティファクト（このリポジトリ自身の解析で zip 約 3.3MB）とキャッシュ 2 種が残ります。
  パブリックリポジトリではどちらも無料ですが、プライベートリポジトリではアーティファクトの容量が
  利用者のストレージの無料枠から引かれます。繰り返し動かす場合は `artifact-retention-days` を
  短くしてください（[保存量と保持期間](#保存量と保持期間)）
