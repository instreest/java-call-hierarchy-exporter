...
| 構文が呼ぶメソッド | 呼び出し式が無くても JLS が「呼ぶ」と定めるものは辺にする。拡張 for 文の `iterator()` / `hasNext()` / `next()`（JLS 14.14.2）、try-with-resources の `close()`（JLS 14.20.3）、レコードパターンのアクセサ（JLS 14.30.2）。**文字列変換の `toString()`（JLS 5.1.11。`"x" + obj`）は辺にしない**（`Object.toString` の全実装が候補になるだけで絞れず、javac も呼び出し命令を書かない） |
...
| 呼び出し先を修飾する型 | C 行の呼び出し先は**宣言した型**で持つ。javac は受け手の静的な型で修飾する（JLS 13.1）ので、`Plain p; p.greet()`（`greet` は親インターフェース `Greeter` のデフォルトメソッド）の候補には、`Plain` の部分型でない `Greeter` の実装も入る。多すぎる側（安全側）の近似である |
...
| 起動の入口 | 名前が `main` で引数が `String[]` か無し、private でないメソッドを入口（`FRAMEWORK_ENTRY`）にする（JLS 12.1.4。インスタンスメソッドの `void main()` を含む） |
