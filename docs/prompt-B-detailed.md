...
1 行目は `<形式バージョン><TAB>source=<準拠レベル><TAB>enc=<ソースの文字コード><TAB>jdk=<java.specification.version><TAB>jdt=<JDTの版>`。
...
F  相対パス  サイズ  エラー数  内容ハッシュ  構文エラー数  未解決数  crc
   ...
   crc はこのブロックの残りの行（F 行の次から、次の F 行・Z 行の手前まで。各行に \n を付けたバイト列）の CRC32
I  依存する型（カンマ区切り）  型の形の指紋  解決できなかった名前（カンマ区切り）
   ...
   型の形の指紋は、宣言する型の、継承したものを含むメンバーの署名（JDT のキー・修飾子・戻り値と引数の型・可変長引数か・
   throws の型と検査例外か）・親型・型引数を並べ替えてハッシュにした頭 16 文字（java.* の型のメンバーは辿らない。無い型を
   復元したものには印を付ける。私的メンバーは、それを宣言した型の親型に私的でない同じ名前のメンバーがあるときだけ入れ、
   私的なコンストラクタは入れない。cache-unification-qa.md の Q51）。
   依存する型には、呼び出し先の throws の型、拡張 for 文・switch のセレクタ・throw の式の型、アノテーションの型、
   switch の case に書いた型とその推移的な親も入る。型解決に失敗したブロックでは、依存する型すべての推移的な親も入れる。
