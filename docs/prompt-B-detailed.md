...
- キャッシュは出力フォルダに置かず、ツール自身のプロジェクトフォルダ（作業ディレクトリとその上位、次に実行中の
  ...
  プロジェクトごとのサイドカーとして置く。中身は **2 ファイル**で、`analysis-cache.tsv`（呼び出し階層を
  出すための事実）と `dataflow-cache.tsv`（呼び出し階層には要らない、データフローのための事実。
  文字列の長さに上限を設けない）。同じ `project.root` を指す設定は同じキャッシュを共有する
...
```
フェーズ1  ソース解析   .java（1ファイルずつ）→ キャッシュファイル 2 つ（TSV）
フェーズ2  グラフ構築   キャッシュ → メソッドをintのIDに内部化したCSR形式の呼び出しグラフ
フェーズ3  出力         methods.csv を書き、起点ごとに深さ優先で辿りながら call-hierarchy.csv を1行ずつ書く
```

**キャッシュは 2 ファイルに分かれる**（2.14）。`analysis-cache.tsv` だけで呼び出し階層は出せ、
`dataflow-cache.tsv` は値の追跡のための事実を持つ。両者は常に同じ実行で一緒に書き、同じブロックを
同じ順で持つ。片方だけが新しい状態は許さない（ズレを許すと、同じソースでも「どちらがどこまで
新しいか」で出力が変わる）。
...
読まずに済むため）。キャッシュの1行目に形式バージョンと準拠レベルを書き、不一致なら
全体を破棄する。更新は旧キャッシュを先頭から読むストリーミングマージで、ランダムアクセスも
全件保持もしない。
...
無いことを区別する）と、レシーバの出所 `r=` を付ける。入れ子の出所が実引数リストを持つ場合は
`{}` で囲む。**入れ子の段数に上限は無い**（`invoke` ← `getMethod` ← `forName`/`getClass` の
連鎖を何段でも読み手が辿れる）。
...
実引数の出所も入れ子にする（`{}` で囲む）。追跡できない引数は載せない。区切りは `|` と `;`
（値の側が括弧を含むため、括弧だと対応の判定が要る）。**要素を走るときは素の `split(";")` を
使わないこと。** `{}` の深さを数えて区切りを探す関数を1つ用意し、必ずそこを通す。入れ子が来た
瞬間に要素が壊れ、壊れ方が「エラー」ではなく「解決が静かに消える」になる。

この上限の無い形は、書き手が文字列として作るのではなく、**1つの式を1ノードとして持つ値グラフ**
（2.14 の N 行）としてキャッシュに書き、読み手が読む直前に上の文字列へ組み直して作る。
文字列に展開すると深さ d・引数 k で k^d に膨らむが、値グラフはノード番号で参照するので
大きさが式の数に比例し、深さに依存しないため。組み直すときは安全弁として文字数の予算を
置き（実在のコードで当たらない大きさ。当たったらそれ以上入れ子を展開せず頭だけにする）、
同じノードを何度も組み直さないよう覚え書きを持つ。
...
- `analysis-cache.tsv` の値にタブ・改行が混ざると形式が壊れるので書き出す前に除去する
  （`dataflow-cache.tsv` は長い文字列を持つので、除去ではなく符号化する。2.14）
...
呼び出し階層（`call-hierarchy.csv` / `methods.csv`）は前者だけで出せる。後者はサイドカーの解析の
ためにあり、**文字列の長さに上限を設けない**。分けたのは、主目的のキャッシュを肥大化させずに
値の追跡を精密にできるようにするため（分けたあとは、値を足しても前者は 1 バイトも動かない）。

どちらに置くかの基準は「**呼び出し階層を出すのに読むか**」。読むなら analysis、読まないなら
dataflow。`analysis` 側だけでは具象クラスの解決は CHA 止まりになる（呼び出しが消えることはない）。

**2 つは常に同じ実行で一緒に書き、片方だけが新しい状態は許さない。** 両方のヘッダに同じ世代の
印（乱数）を書いて突き合わせ、食い違えば両方とも捨てて全件解析する。さらに最終行の
ブロック数（Z 行）も一致を見る。再利用するのは**両方に存在するブロック**だけ。ズレを許すと、
同じソースでも「どちらがどこまで新しいか」で出力が変わってしまう。

### ヘッダと行の形

タブ区切り。1行目 `<形式バージョン><TAB>source=<準拠レベル><TAB>enc=<ソースの文字コード><TAB>jdk=<java.specification.version><TAB>gen=<世代の印>`。
`gen=` を除いた部分が「キャッシュを作ったときの前提」の鍵で、1 つでも違えば丸ごと捨てて全件解析し直す。
**形式バージョンは 2 ファイルで独立に持つ**（データフロー側の事実を足すときは、そちらだけ上げれば
呼び出し階層の出力は動かない）。`F` 行が現れるたび以降の行はそのファイルに属する。
各行の列の並びは、その行を表す record の `toRow()` / `fromRow()` に1箇所で定義する。

`analysis` 側は値にタブ・改行が混ざらないようにする（混ざる値は事実として拾わない）。
`dataflow` 側は長い文字列と制御文字をそのまま持つため、行形式を壊す文字を
バックスラッシュで符号化する（`\\` `\t` `\n` `\r`、その他の制御文字は `\uXXXX`）。
**この符号化は「往復すること」と「符号化後にタブ・改行が残らないこと」をテストで確かめる。**
対象はソースに現れる形（SQL・書式文字列・Windows のパス）だけでなく、符号化の形をした文字列
（`\u0041` という中身の文字列）や途中で切れた符号、サロゲートペアまで含める。

```
L  jarのパス  指紋  パッケージ(カンマ区切り)            解析時の依存 jar。ヘッダ行の直後にクラスパス順。
                                                       パスは project.root 配下なら相対、外なら絶対。
                                                       指紋は中に入っているクラスの一覧から作る（下記）。
                                                       META-INF/ 配下とデフォルトパッケージは除く
F  相対パス  サイズ  エラー数  内容ハッシュ               エラー数は JDT が報告したエラーの件数（解決が
...
Z  ブロック数                                        最終行。ここまで書き終えた印。無い・数が
                                                     合わなければ途中で切れたとみなして捨てる
```

`dataflow-cache.tsv` の行（`F` 行と `Z` 行の意味と並びは `analysis` 側と同じ）:

```
F  相対パス  サイズ  エラー数  内容ハッシュ            （ブロックの先頭。analysis 側と同じ並び・同じ数）
...
J  typeFqn  fieldName  site  origin                   その型自身のフィールドへの代入1件
                                                      ...
                                                      同じブロックの V 行（analysis 側）と組で判定する
K  typeFqn  name  種別(V=値/H=ハッシュ)  値            このファイルが宣言するコンパイル時定数。
                                                      値は使う側に焼き込まれるので、差分更新で
...
N  id  kind  value  recv  args  argCount               値グラフのノード1件。1つの式を1ノードとして
                                                      1回だけ書き、参照はノード番号で行う。
                                                      id はブロック内ローカルの 0 始まりの連番で、
                                                      recv と args は同じブロックのノードを指す。
                                                      入れ子を展開しないので深さの上限が要らず、
                                                      value の長さにも上限が無い（符号化して書く）
P  line  callerPkg callerType callerMethod callerParams  calleeName  ordinal  recv  args  recvKey  guard
                                                      呼び出し箇所ごとの値。recv / args は N 行の
                                                      ノード番号。guard は呼び出し箇所を囲む条件分岐
Z  ブロック数                                          analysis 側と同じ数でなければ対になっていない
```

**N 行の番号はブロック内ローカル。** ブロックをまるごと書き写す差分更新で参照が壊れないよう、
ブロックの外を指すことは無い。同じソースなら同じ番号になる（決定的）。
同じ構造のノードは1つにまとめる。**「番号がブロックごとに 0 から詰まっている」「recv と args が
同じブロックの範囲に収まっている」をテストで確かめる**（崩れるとブロックを書き写した瞬間に
参照がずれる）。

**P 行と C 行・U 行の結びつけ**は、同じブロックの中で**同じ順に書く**ことで位置で取る。ただし
位置だけに頼ると並びを変えた瞬間に静かにずれるので、P 行に自己記述の鍵（行番号・呼び出し元・
呼び出し先の表示名・同じ鍵の中での通し番号）も持たせ、取った P 行の鍵が一致することを毎回
確かめる。食い違えば**値を使わない方に倒す**（呼び出しは消えず、解決が CHA 止まりになるだけ）。
候補の無い U 行は読み飛ばすが、**読み飛ばす行でも位置は1つ進める**。通し番号が要るのは
`f(g(), g())` のようにまったく同じ鍵が同じ行に複数並ぶことがあるため。

読み手は `dataflow` 側を**ブロック単位で前へ読み進めるだけ**の形で開き、`analysis` 側の F 行に
出会うたびに同じパスのブロックへ進む。2 つは常に同じブロックを同じ順で持つので、前へ進むだけで
対応が取れ、**ヒープに載るのは 1 ブロック分だけ**で済む。ただし `J` 行は順番に注意が要る。
フィールドへの代入は同じブロックの `V` 行（`analysis` 側の宣言）が揃ってからでないと拾われない
ので、ブロックの先頭で読んだ `J` 行は溜めておき、ブロックを読み終えた時点で渡す。先に渡すと
宣言が未登録で捨てられ、**コンストラクタ注入の解決が静かに消える**。

C行とU行はソース上の順のまま1つの列として書く（読み手が import推定の候補をエッジにしたとき、
元の呼び出しの並びが保たれる）。型解決に失敗した呼び出しがインスタンス初期化子の中にあれば、
C行と同じく根のコンストラクタごとに1行になる。

事実の収集範囲（書き手の打ち切り。変えたら版を上げる）: **値グラフ（N 行）の入れ子には段数の
上限が無く、文字列・定数の値にも長さと内容の上限が無い**（1つの式を1ノードとして持ち、参照は
ノード番号で行うので、大きさが式の数に比例し深さに依存しないため）／外側スコープの変数は
final か実質 final のときだけ／ローカル変数の先読みは表が変わらなくなるまで繰り返す（上限の回数を
使い切ったら書いた変数は U）／プリミティブ・配列・String を返す
return は記録しない／フィールドへの代入はその型自身のメソッド・コンストラクタ本体とフィールド
初期化子から拾う（インスタンス初期化ブロックと内部クラスからの代入は拾わない）。

**値グラフの葉でローカル変数を扱うときは、出所の文字列に落とさずノード番号で指すこと。**
文字列に落とすと実引数リストが剥がれ、`Class.forName(NAME)` を受けたローカル変数から
クラス名が辿れなくなって**リフレクションの解決が静かに消える**。変数の表（バインディングキー →
出所）と並べて、同じキーでノード番号の表も持つ。ただし**外側のメソッドから捕捉した変数は
...
1. 旧キャッシュを読み、サイズと内容ハッシュが一致するファイル（有効）を覚える。無効・消滅した
...
ヒープ常駐は「ソースファイルの一覧＋サイズ」と
「変わった型・パッケージの集合」だけ。

差分更新は 2 ファイルを同じパスで進める。旧キャッシュを読む前に**対になっているか**
（ヘッダの世代の印と Z 行のブロック数）を見て、対でなければ両方捨てる。対であっても、
**両方に存在するブロックだけ**を再利用の候補にする（片方に無いブロックは再解析に回す。
これで対の不変条件が自己修復する）。書くときは両方の一時ファイルへ同時に書き、両方そろって
から本物に差し替える。中断した実行から引き継ぐのも「両方の一時ファイルにそろっているブロック」
だけにする。

**列を変えたら必ず形式バージョンを上げること。** 上げ忘れると古いキャッシュが互換とみなされて
列がずれたまま読まれ、壊れ方が「エラー」ではなく「静かに違う結果」になる（参照実装で実際に
起きた。`recvKey` を `recvKind` として読んでいた）。**ヘッダの版だけを書き換えたキャッシュを
再利用せず捨てることをテストで確かめる**（2 ファイルそれぞれについて）。

なお、この節が書いているのはキャッシュの骨格（分割の考え方、差分更新の考え方と5パス）で、
行の種別はこれ以外にもある（ソース一覧の指紋の T行など）。
