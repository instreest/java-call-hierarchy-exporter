...
**依存を 1 段辿るだけでは足りない場合が 3 つある**（2.14 の「差分更新」の 3・4）。コンパイル時定数の値は使う側に
焼き込まれるので、値が変わったら定数を宣言する型を使うファイルへ連鎖させる。親の親の変化（継承したメンバー）は
子の利用者の依存に載らないので、変わった型の部分型もすべて（推移的に）変わった型にする（H 行から作る部分型の索引で
引く）。前回は無かった型・見えなかった型は、参照していた側の依存に残らないので、
...
I  依存する型（カンマ区切り）  解決できなかった名前（カンマ区切り）
   ...
   (b) すべての式（名前・呼び出し・ラムダ・メソッド参照・アノテーションを含む）の型（resolveTypeBinding）と、すべての型の節
       ...
       型変数・ワイルドカード・交差型は上限の消去で数える（上限が複数なら全部）。ただし、
       ...
       同じ名前のメソッド（またはコンストラクタ）すべての引数の型（型変数は上限の消去）。java.* の型が宣言するものは見ない。
...
1. 旧キャッシュを読み、サイズと内容ハッシュが一致し、crc も合うファイル（有効）を覚える。無効・消滅した
   ...
   回復は同じパッケージにあると推定した名前を残す）。定数の連鎖のために各ブロックの K 行の指紋を、新しい型のために
...
3. 依存の索引を読み、有効なブロックのうち I行が「変わった型」か「変わったパッケージ」に触れるものを再解析に回す
   ...
   パッケージの型として隠しうるが、どの型が増えたかは分からない）
4. 再解析に回したファイルを解析して追記する（H 行の親型は部分型の索引に足す）。そのファイルが宣言する定数（K 行）の値が
   旧キャッシュと違っていたら、宣言する型を「変わった型」に加える（定数の値は使う側に焼き込まれるので、依存を 1 段
   辿るだけでは足りない）。「変わった型」が増えていれば 3 へ戻る。
   **親型の連鎖**: 型を「変わった型」に加えるときは、部分型の索引にあるその部分型も推移的に「変わった型」に加える。
   索引に関係（子 -> 親）を足すときも、親がすでに「変わった型」か変わった jar のパッケージの型なら、子（とその部分型）を
