...
`plugin.folders` 配下のファイルの相対パス・サイズ・内容ハッシュ）をキャッシュのヘッダ行に入れ、
変われば全件解析し直す。
...
タブ区切り。1行目 `<形式バージョン><TAB>source=<準拠レベル><TAB>enc=<ソースの文字コード><TAB>jdk=<java.specification.version>`。この 1 行が「キャッシュを作ったときの前提」の鍵で、1 つでも違えば丸ごと捨てて全件解析し直す。
...
jar の順序だけの変化は検知しない。ヒープ常駐は「ソースファイルの一覧＋サイズ」と
「変わった型・パッケージの集合」だけ。

なお、この節が書いているのはキャッシュの骨格（差分更新の考え方と5パス）で、行の種別は
これ以外にもある（コンパイル時定数の K行、ソース一覧の指紋の T行、書き終えた印の Z行）。
