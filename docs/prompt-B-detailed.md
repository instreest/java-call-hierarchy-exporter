...
解決結果が依存する型（呼び出し先・フィールドの所有型・親型・引数型・import の型のほか、ソースに書かれた型の名前、
ラムダの目標の型、呼び出しの受け手と実引数の式の型など。一覧は 2.14 の I 行）を宣言するファイルが変わっていれば
...
子の利用者の依存に載らないので、解析し直したファイルの型の形（継承したものを含むメンバーの署名・親型）が前回と
違えば、その型を使うファイルへ連鎖させる。前回は無かった型・見えなかった型は、参照していた側の依存に残らないので、
...
I  依存する型（カンマ区切り）  型の形の指紋  解決できなかった名前（カンマ区切り）
   ...
   (a) このファイルのバインディングから名前にした型すべて（呼び出し先・フィールドを宣言した型、引数の型、親型、宣言の型など）
   (b) ソースに書かれた型の名前（SimpleType・QualifiedType・NameQualifiedType の節。戻り値・throws・ローカル変数・キャストの型）
   (c) ラムダ・メソッド参照の目標の関数型インターフェースとその推移的な親
   (d) すべての呼び出し（解決できたかどうかに関わらない）の受け手の式と実引数の式の型、new する型とその外側のインスタンスの
       式の型、super(...) の外側の式の型、メソッド参照の左側の式の型
   (e) 修飾された名前・フィールド参照の左側の式・名前の型（a.getB().count の B。解決できなかった名前も）。
       (d)(e) は型変数・捕捉された型変数・ワイルドカード・交差型を上限の消去で数える
   (f) 呼び出したメソッド・コンストラクタの throws の型（型変数は上限の消去）
   (g) 拡張 for 文の式・switch のセレクタ・throw の式の型
   (h) アノテーションの型（宣言のほか、引数・ローカル変数・型の使用に付いたもの）
   (i) switch の case に書いた列挙定数の型と型パターン（レコードパターンを含む）の型、とその推移的な親
   (j) import 文の型（オンデマンド import は "pkg.*"）
   (k) 型解決に失敗したブロック（エラーか BINDING_FAILED の U 行がある）では、名前にした型（自分が宣言する型を含む）すべての
       推移的な親
   (f)(h)、(i) の親、(k) は java.* の型を数えない（JDK の版は鍵に入っている。宣言に付いた java.lang 以外のアノテーションは
   H・D・V 行に書くので (a) として載る）。
   型の形の指紋は、このファイルが宣言する型（入れ子・匿名・ローカルを含む）ごとの行を並べ替えてハッシュにした頭 16 文字
   （宣言する型が無ければ空）。行は、型の JDT のキー・修飾子・種類、推移的な親型（キー・修飾子・種類）、型引数とその上限、
   自分と親型が宣言するメソッド（キー・修飾子・戻り値と引数の型・可変長引数か・throws の型と検査例外か）・フィールド
   （キー・修飾子・型）・入れ子の型（キー・修飾子・種類）。java.* の型のメンバーは辿らない。無い型を復元したものには印を
   付ける。私的メンバーは、それを宣言した型の親型（推移的。java.* の親型とそのさらに上も含む。インターフェースは Object の
   メソッドの名前も）に私的でない同じ名前のメンバー（種類は問わない）があるときだけ
   入れ（親のメンバーを隠す・継承を止めるものだけが外から見える違いを生む。JLS 6.6.1・8.2・8.3・8.4.8・8.5。
   `Registry extends HashMap` の私的な `Entry` は `HashMap` ではなく `Map` の `Map.Entry` を隠す）、
   私的なコンストラクタは入れない（私的でないコンストラクタは入れる。cache-unification-qa.md の Q51・Q52）。
   解決できなかった名前は、型解決に失敗したブロックでだけ書き（失敗していなければ空）、エラーの引数（IProblem.getArguments）に
   現れた点区切りの識別子（Foo・org.missing・q.Bar）。エラーの種類では絞らない（取りこぼさないため）が、パスの区切り
   （/ か \）を含む引数は拾わない（型が重複しているエラーはソースファイルの絶対パスを引数に入れるので、拾うとチェックアウトの
   場所のフォルダ名がキャッシュに入る。cache-unification-qa.md の Q64）。1 つも拾えなければ "*"
...
3. 依存の索引を読み、有効なブロックのうち I行が「変わった型」か「変わったパッケージ」に触れるものを再解析に回す
   ...
   どれかが変わった型の単純名か、名前が変わった型の FQN の頭の部分。"*" は何にでも当たる。無い型・見えない型の名前は
   依存には残らない。周回ごとに見直す）と、**新しいトップレベルの型と同じパッケージで、I 行の型名にその単純名を含む
   ブロック**（同じパッケージの型はオンデマンド import と `java.lang` を隠す。JLS 6.4.1）も回す。変わった jar の
   パッケージが自分のパッケージなら、I 行にオンデマンド import か `java.lang` の型があるブロックも回す（jar の型も同じ
