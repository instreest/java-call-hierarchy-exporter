...
| `library.folders` | 空 | 依存jarを集めたフォルダ（カンマ区切り）。フォルダ直下の `*.jar` を全部使う。`.classpath` の `kind="lib"` があれば合算。jarを足す・差し替える・外すと、次回の実行でその jar のパッケージを参照しているファイル（足す・差し替えるときは型解決に失敗していたファイルも）だけが解析し直される。**空なら `pom.xml` / `build.gradle` を読んでローカルリポジトリから集める（5.8）** | `project.root` |
...
読まずに済むため）。キャッシュの1行目に形式バージョンと準拠レベル（と文字コード・実行 JDK・JDT の版・ソースフォルダの
並び）を書き、不一致なら全体を破棄する。更新は旧キャッシュを先頭から読むストリーミングマージで、全件保持はしない。

**依存先の変更でも再解析する。** そのファイル自身が変わっていなくても、そのファイルの
解決結果が依存する型（呼び出し先・フィールドの所有型・親型・引数型・import の型のほか、すべての式と型の節の型。
一覧は 2.14 の I 行）を宣言するファイルが変わっていれば
解析し直す。オーバーロードの追加やフィールドの改名で、呼んでいる側の解決結果が変わるため。
**依存を 1 段辿るだけでは足りない場合が 3 つある**（2.14 の「差分更新」の 3・4）。中身の変わっていないファイルを
解析し直した結果、宣言（宣言に書いた型の解決先。同じパッケージに足した型がオンデマンド import を隠す）かコンパイル時定数の
値（使う側に焼き込まれる）が変わったら、そのファイルの型を使うファイルへ連鎖させる（自分の宣言の指紋で見る）。親の親の
変化（継承したメンバー）は子の利用者の依存に載らないので、変わった型の部分型もすべて（推移的に）変わった型にする
（H 行から作る部分型の索引で引く。jar の変化で解析し直したファイルの型も変わった型にする）。前回は無かった型・見えなかった
型は、参照していた側の依存に残らないので、
型解決に失敗していたファイルのうちエラーに現れた名前が変わった型に当たるものと、同じパッケージに足した型に名前を
隠されうるものを解析し直す。ログには `新規解析=N（うち依存先の変更による再解析=M）` と出す。

**依存 jar の変更も検知する。** キャッシュに解析時の依存 jar（パス・中身の指紋・含まれる
パッケージ）と、ファイルごとに JDT が報告したエラー数を残す。次回の実行で jar が追加・
差し替え・削除されていれば、その jar のパッケージの型を参照しているファイルを解析し直す。追加・差し替えの
ときだけは、前回型解決に失敗していたファイルも解析し直す（無い型の名前は依存に残らないので、パッケージでは
当たらない。削除だけなら解決できる型は増えず、失敗していた型解決が成功に変わる理由にならない）。実行する JDK が変わったときはキャッシュ全体を作り直す。
...
**DI（段5）は注入点だけに当てる。** コンテナが値を入れるのは注入点だけで、それ以外のフィールド・引数には利用者の
コードが何を入れてもよい。Bean でないクラスの static メソッドの引数や、利用者が `new` して渡したフィールドまで唯一の
Bean に確定すると、実際に渡した実装への呼び出しを黙って落とす（参照実装で実際に起きた）。注入点は次のとおり。

| レシーバ | 注入点とみなす条件 |
|---|---|
| フィールド | 値が今のオブジェクトのフィールド（`F:型#名前`。`other.f` は値にしない。2.10）で、`@Autowired` / `@Inject` / `@Resource` が付いているか、ステレオタイプ注釈の Bean（またはその部分型が Bean である基底クラス）の static でないフィールド |
| 引数 | 呼び出しを書いたメソッドが `@Autowired` / `@Inject` / `@Resource` を付けたメソッド・コンストラクタ、`@Bean` メソッド、ステレオタイプ注釈の Bean（またはその基底クラス）のコンストラクタで、値がその引数（`A:n`。本体で書き換えた引数は外れる）。ラムダの本体の引数は含めない |

値を読まない指定（`dataflow.enabled=false`）では、フィールドは「呼び出しを書いたメソッドの型が Bean（またはその基底）か、
その型か親に注入の注釈の付いたフィールドがある」とき、引数は上と同じメソッドの条件で注入点とみなす（粗いが、値を
読まない指定でも DI の絞り込みを効かせる）。候補の型に、返す具象型の決まらない `@Bean` メソッドの Bean がなりうる型が
あれば絞らない（2.10 (d)）。段5で絞った呼び出しも、経路で実際に渡った値の具象型が分かればそちらを採る（Bean の
コンストラクタも利用者のコードが直接呼べるため。段1〜4 で 1 つに決まった結論は DI の推定で覆さない）。
`@Qualifier` / `@Resource(name)` の指名を読むのは、注釈の付いたフィールドだけ。
...
| `F:typeFqn#field` | フィールド（今のオブジェクト＝this のインスタンスフィールドか static フィールド） | コンストラクタ注入の記録と経路上の生成箇所を見たとき |
| `O:typeFqn#field` | `this` 以外で修飾したインスタンスフィールド（`other.dao`・`Outer.this.dao`）。コンストラクタ実引数は当てない | フィールドの値の頭が、どのインスタンスでも同じ `new` の型か、捕捉した引数を使わないラムダ・メソッド参照と分かったとき |
| `L:値` | 文字列リテラル／文字列に評価されるコンパイル時定数（長さ・形の制限なし。SQL やログ文言もそのまま） | その場 |
| `V:値` | 条件分岐の判定に使う定数の値。真偽値・整数・文字（数値で持つ）のコンパイル時定数と、列挙定数（宣言型で修飾した名前 `fx.Mode.FULL`）。浮動小数は持たない（`U`）。文字列の定数は `L` のノードで、条件の subject にするときだけ、64 文字以内で制御文字を含まないものを `V` のノードにする | その場 |
| `K:fqn` | クラスリテラル `X.class`（配列は `[]` 付き、プリミティブはそのまま） | その場 |
| `C:n` | `Class.forName(n番目の引数)` で名前指定された型 | その呼び出しの実引数を見たとき |
| `Z:typeFqn#m(params)` | 関数型インターフェースの値として渡したラムダ式・メソッド参照。値は実際に動くメソッドのキー（ラムダは本体の合成メソッド、メソッド参照は参照先）。束縛したレシーバ（`dao::describe` の `dao`）は `r=` に持つ。具象「型」ではなく具象「メソッド」が決まる唯一の出所 | その場 |
| `E:n` | ラムダ式が捕捉した、囲みメソッドの n 番目の引数（合成メソッドの中から見ると自分の引数ではないので `A:` と分ける） | ラムダを生成した箇所の辺を降りたとき（そのフレームの引数を捕捉した値として渡す） |
| `U` | 追跡できない | 決まらない |
...
実引数の位置は、宣言どおりの呼び出し先と実際に降りる先が違うときに合わせる。型名で書いたメソッド参照
（`Mode::chk` を `BiConsumer<Mode, Mode>` にして `c.accept(B, A)`）へ降りるときと `Method.invoke(obj, a, b)` から
降りるときは、第 1 実引数がレシーバなので 1 つずらす（JLS 15.13.3）。引数の数が同じならずらさない。降りる先の最後の
引数が配列（可変長引数にまとめられているかもしれない）か、それ以外で数が合わなければ環境を渡さない。
...
| b | 書き込みがコンストラクタ本体かフィールド初期化子の中だけ（J 行の site が `?` の書き込みが 1 つでもあれば崩れる） | setter、内部クラス・初期化ブロックからの書き込み、条件の中の書き込み |
...
持ち回る。レシーバの由来が this の呼び出し（修飾の無い `m()` と `this.m()`）は同じオブジェクトなので引き継ぐ。
レシーバの値が追えないだけの呼び出し（拡張 for の変数・パターンの変数・配列の要素への呼び出し）は this とは限らない
ので引き継がない。フィールドの読み取りも、今のオブジェクトのもの（修飾の無い名前と `this.f`）だけに当てる。
...
- `invoke`: レシーバの出所が `Class#getMethod` / `getDeclaredMethod` の戻り値で、その受け手
  ...
  その型と親の型すべてから集めて候補にする（`getMethod` は継承したメソッドも返す。親の private は除く。
  `getDeclaredMethod` はその型が宣言したものだけ）。`invoke` の第1引数の具象型が受け手のサブタイプなら
  その型から実装を引き直す。ただし見つけたメソッドが private か static なら引き直さない（上書きされない・
  受け手を使わない）
...
1 行目は `<形式バージョン><TAB>source=<準拠レベル><TAB>enc=<ソースの文字コード><TAB>jdk=<java.specification.version><TAB>jdt=<JDTの版><TAB>folders=<ソースフォルダの一覧>`。
ソースフォルダの一覧は、project.root からの相対パス（`/` 区切り。project.root そのものは `.`）を並びのままカンマでつないだもの。
名前は列の符号化（下の「列の符号化は全列に同じ 1 つの規則」）をしたうえで、カンマと空白も `\u002c`・`\u0020` にする（ヘッダ行は符号化しない行で、
読むときに端の空白を削るため）。これが「キャッシュを作ったときの前提」の鍵で、1 つでも違えば丸ごと捨てて全件解析し直す。
ただしソースフォルダの一覧だけが違い、両方にあるフォルダが同じ順に並び、どちらの一覧のフォルダも別のフォルダの中に
無いなら使い続ける（同じ名前の型が 2 つのソースフォルダにあると JDT はソースパスの先勝ちで解決するので、並びを入れ替えれば
同じソースでも解決先が変わる。足した・外しただけなら、そのフォルダのファイルを足した・消したファイルとして扱える。
入れ子のフォルダはコンパイル単位の名前を変えるので扱えない）。中断した実行の一時ファイルを引き継ぐときは、1 行目が
丸ごと同じときだけにする。JDT の版は
...
I  依存する型（カンマ区切り）  解決できなかった名前（カンマ区切り）  自分の宣言の指紋
   ...
   依存する型（FQN の昇順。自分が宣言する型は含まない。配列は要素の型、プリミティブは除く）は次の和。
   (a) このファイルのバインディングから名前にした型すべて（呼び出し先・フィールドを宣言した型、呼び出し先の引数の型、
       親型、宣言の型など）
   (b) すべての式（名前・呼び出し・ラムダ・メソッド参照・アノテーションを含む）の型（resolveTypeBinding）と、すべての型の節
       （Type）の型（resolveBinding）。AST の訪問の前処理（ASTVisitor.preVisit2）で 1 つの決まりとして拾う。型変数・捕捉された
       型変数・ワイルドカード・交差型は上限の消去で数える（上限が複数なら全部）。型引数も数える（List<Foo> の Foo。
       上限と同じく深さの上限まで。a.foos() の型 List<Foo> の Foo はソースに名前が無く、Foo の親を変えると
       for (Bar b : a.foos()) や m(Collection<? extends Bar>) の解決が変わる。cache-unification-qa.md の Q85）。ただし、
       - JDT が解決できなかった名前（Name の節で、resolveBinding が null か isRecovered()）は数えない。依存 jar が無いとき、
         式の中の org.missing.pkg.Type.run() の org.missing に JDT が回復した型（org.missing）を返すかは、同じバッチで
         先に別のファイルが同じ名前を型の文脈（org.missing.pkg.Type.class など）で解決しようとしたかで変わる（JDT は
         回復のために作った型をバッチの中で使い回す）。数えると I 行がバッチの組み方で変わる（cache-unification-qa.md の Q79）。
         型の節は回復した型でも数える（型の文脈の回復はバッチに依らない。途中のパッケージができる・無くなると変わるが、
         それは下の「差分更新」の 3 の「できた・無くなったパッケージ」で拾う。Q80）
       - アノテーションの型は java.* のもの（@Override など）を数えない（JDK の版は鍵に入っている）。アノテーションの
         型の名前の節は、アノテーションの節の側で数える
       あわせて、呼び出し・メソッド参照・new・super(...) の節では、呼び出しの候補（JLS 15.12.2）の引数の型も数える。探す型は、
       修飾する式・型があればその型、単純名の呼び出しと super.m() なら囲む型すべてと static import した型（import static
       p.X.m の m が同じ名前か、import static p.X.*）、new とコンストラクタ参照なら作る型、super(...) なら親のクラス
       （this(...) の候補は自分のコンストラクタなので見ない）。その型と推移的な親型（コンストラクタは探す型だけ）が宣言する、
       同じ名前のメソッド（またはコンストラクタ）すべての引数の型（型変数は上限の消去。型引数も数える）。探す型は型引数を
       付けたまま辿る（Box<Foo> の put(T) は put(Foo)。親型も getSuperclass の型引数を置き換えた型）。java.* の型が
       宣言するものは、型引数を置き換えた引数の型のうち java.* でない型だけを数える（S extends ArrayList<Foo> の
       s.add(0, null) の add(int, E) の Foo。Q85）。
       選ばれなかった候補の型も、どの候補が選ばれるか（曖昧か）に効く（x.m(null) は m(Y) と m(Z) で曖昧だが、Z を消すと
       m(Y) に決まる。ラムダを渡すなら、候補の関数型インターフェースの形で決まる）。選ばれた候補の引数の型は (a) で載るが、
       選ばれなかった候補の型はソースのどこにも無い（cache-unification-qa.md の Q78）
   (c) 呼び出したメソッド・コンストラクタの throws の型（型変数は上限の消去。java.* の型は数えない。検査例外かどうかで
       呼ぶ側のエラーが変わるが、throws の型は呼ぶ側のソースに無いことが多い）
   (d) import 文の型（オンデマンド import は "pkg.*"。import static a.B.c なら a.B も）
   (e) (a)〜(c) で数えた jar の型（ソースの無い（ITypeBinding.isFromSource が false の）、java.* でない型）の推移的な親型
       （java.* の型で止める）。jar の型には H 行が無いので、l1.Mid（l1.jar）を使うファイルは、親の l2.Top（l2.jar）が
       変わっても、これが無いと解析し直されない（Q86）
   親型の変化は、差分更新が H 行から作る部分型の索引で拾う（下の「差分更新」の 4）ので、(a)(b) で名前にしたもの以外の
   親型は数えない。
   解決できなかった名前は、型解決に失敗したブロックでだけ書き（失敗していなければ空）、エラーの引数（IProblem.getArguments）に
   現れた点区切りの識別子（Foo・org.missing.Lib・q.Bar）。エラーの種類では絞らない（取りこぼさないため）が、パスの区切り
   （/ か \）を含む引数は拾わない（型が重複しているエラーはソースファイルの絶対パスを引数に入れるので、拾うとチェックアウトの
   場所のフォルダ名がキャッシュに入る。cache-unification-qa.md の Q64）。拾った名前が、エラーの位置（getSourceStart から
   getSourceEnd。NodeFinder で節を引き、型の節ならその名前、名前なら親の名前を辿った名前全体）に書かれた名前の頭の部分
   （同じか、頭から点の区切りまで一致する）なら、書かれた名前全体に置き換える（上と同じ理由で、同じ式のエラーが
   org.missing cannot be resolved にも org.missing.pkg.Type cannot be resolved to a type にもなるので、書かれた名前
   org.missing.pkg.Type にそろえる。Q79）。1 つも拾えなければ "*"。
   自分の宣言の指紋は、このファイルが宣言する型（匿名・ローカルの型も）ごとに、型のバインディングの鍵・修飾子・直接の親型の
   鍵（型引数を含む）・型引数とその上限の鍵、型が宣言するメソッド（getDeclaredMethods。暗黙のコンストラクタ・レコードの
   アクセサも）の鍵・修飾子・可変長引数か・戻り値と引数と throws の型の鍵・型引数とその上限の鍵、フィールド
   （getDeclaredFields）の鍵・修飾子・型の鍵を 1 行ずつ作り、K 行の指紋と合わせて並べ替えてハッシュにしたもの。継承したものは
   入れない（親の変化は部分型の索引で届く）。下の「差分更新」の 4 が前回と比べる（Q83）
...
G  ガード番号  op  subject  text  値1  値2 …
   ...
   含まないもの。浮動小数は条件にしない）で、1 列に 1 つずつ置く
...
J  typeFqn  fieldName  site  node
   フィールドへの書き込み1件（node は代入した値。複合代入と ++ / -- は -1）。site は "<field>"=初期化子（インスタンス
   初期化ブロックの書き込みも）か "name(paramSig)"=そのコンストラクタ・メソッドか "?"。"<field>" とコンストラクタにするのは
   生成のたびに必ず通る this への書き込み（本体の直下の式文で、前に return が無い）だけで、条件・ループ・try・ラムダ・
   入れ子の型の中、this 以外のインスタンス（other.f・Outer.this.f）、static 初期化ブロック、別の型の本体からの書き込みは
   "?"（読み手はコンストラクタでも初期化子でもない書き込みとして扱い、そのフィールドを決まった値とみなさない）
```
...
return は記録しない／**フィールドへの書き込みは漏れなく拾う**（その型自身のメソッド・コンストラクタ・初期化ブロック
（static を含む）・フィールド初期化子とその中のラムダ、別の型の本体（入れ子のクラス・外側のクラス）から private な
インスタンスフィールドへの書き込み。private なフィールドに書けるのは同じコンパイル単位の中だけなので、宣言した型の
事実として同じブロックに書ける。1 つでも取りこぼすと、読み手がコンストラクタで入れた値だけが入ると言い切って呼び出しを
落とす。匿名・ローカルクラスのフィールド初期化子は囲むメソッドの枠から切り離して読む。本体の中で書き換える引数を右辺が
読む代入は値が分からない）／インスタンスフィールドの読み取りを今のオブジェクトのフィールド（`F:`）にするのは、修飾の無い
名前と `this.f` だけ（`other.f` を `F:` にすると、今のオブジェクトのコンストラクタ実引数を別のインスタンスに当てる）。
`this` 以外で修飾したインスタンスフィールドの読み取りは別の種別 `O:`（型#名前）にし、読み手はコンストラクタ実引数を当てず、
どのインスタンスでも同じになる値（フィールドの値の頭が `new` の型か、捕捉した引数を呼び出しに使わないラムダ・メソッド参照）だけを使う
（捕捉した引数を使うラムダは、今のフレームの引数を捕捉した値として当ててしまう）。
`this.m()` のレシーバの由来は修飾の無い `m()` と同じ this／拡張 for の変数の値（要素の出所）は、その本体で引数なしの
`new` をした `java.util` のローカル変数で、再代入せず、要素を足すメソッドのレシーバ・拡張 for の式・要素を足さず外へも
漏らさない問い合わせにしか使わないコレクションだけから作る（コンストラクタの実引数・`addAll`・別のメソッドへ渡す・別名・
引数のコレクションなどで入った要素を見落とすため）。
...
中身が変わったブロック（書き換え・化け）は F 行の crc が合わないことで見分け、全件ではなく、ソースが変わったブロックと
同じ扱いにする（そのファイルを解析し直し、ブロックが宣言していた型（H 行）を「変わった型」にして、その型を使うファイルも
解析し直す。ほかのブロックは再利用する）。

**差分更新**（旧キャッシュを先頭から丸ごと読むのはパス1 の 1 回だけ）:
0. 旧キャッシュのヘッダとL行（解析時の依存 jar）を読み、今回のクラスパスと突き合わせる（ヘッダが違う・先頭の行の
   検査値（T 行の最後の列）が合わなければ丸ごと捨てる）。
...
1. 旧キャッシュを読み、サイズと内容ハッシュが一致し、crc も合うファイル（有効）を覚える。無効・消滅した
   ...
   回復は同じパッケージにあると推定した名前を残す）。宣言の連鎖のために有効なブロックの I 行の自分の宣言の指紋を、新しい型のために
   型解決に失敗していたブロックの I 行の解決できなかった名前を覚える。**どのブロック（有効なものも無効なものも）の H 行の
   親型も、部分型の索引（親型 -> それを親に持つ型）に足す**（下の 4）。H 行のパッケージを「前回のパッケージ」に、有効な
   ブロックの H 行のパッケージを「今回のパッケージ」に足す（下の 3。2・4 で解析したブロックのものも今回のパッケージに足す）。
...
2. 変更・追加されたファイル（1で外したファイルを含む）を解析して新キャッシュへ書く。H 行の親型は部分型の索引に足す。
   そのファイルが宣言する型も「変わった型」に加える（改名・追加に備える。jar 追加で引数型の解決が変わった
   宣言を呼ぶ側にも波及させる）。変更・追加されたファイルが宣言する型のうち、1 を読み終えたときの「変わった型」
   （無効になったブロックの H 行の型と、その部分型）に無かったものは「新しい型」として覚える
3. 依存の索引を読み、有効なブロックのうち I行が「変わった型」か「変わったパッケージ」に触れるものを再解析に回す
   （`pkg.*` はそのパッケージの型が1つでも変われば触れているとみなす。パッケージの照合は I行の FQN の "." 区切りの
   前方部分を全部試す）。**型解決に失敗していた有効なブロックのうち、I 行の解決できなかった名前が変わった型（新しい型を
   含む。名前を持つ型だけ。無名・ローカルの型は名前で参照できないので当たらない）に当たるもの**（名前の点区切りの
   どれかが変わった型の単純名か、名前が変わった型の FQN の頭の部分か、名前の頭の部分（名前そのものは除く）が「できた・
   無くなったパッケージ」か。できた・無くなったパッケージは、前回のソースのパッケージ（旧キャッシュのすべてのブロックの
   H 行のパッケージとその頭の部分）と今回のパッケージ（有効なブロックと、今回解析した・引き継いだブロックの H 行の
   パッケージとその頭の部分）の片方にだけあるものと、変わった jar のパッケージとその頭の部分。パッケージができる・
   無くなると、JDT が org.missing.pkg.Type のどこまでをパッケージとして読むか（エラーと回復した型の名前
   org.missing・org.missing.pkg）が変わる。既にあるパッケージに型を足しただけでは当たらない。"*" は何にでも当たる。
   無い型・見えない型の名前は依存には残らない。周回ごとに見直す）と、**新しいトップレベルの型と同じパッケージで、
   I 行の型名にその単純名を含むブロック**（同じパッケージの型はオンデマンド import と `java.lang` を隠す。JLS 6.4.1）
   も回す。変わった jar のパッケージが自分のパッケージなら、I 行にオンデマンド import か `java.lang` の型があるブロックも回す（jar の型も同じ
   パッケージの型として隠しうるが、どの型が増えたかは分からない）。I 行の型（オンデマンド import はパッケージ名）の名前の
   頭の部分（名前そのものは除く）が「変わった型」のブロックも回す（パッケージ a.b があるのに足したパッケージ a のクラス b は、
   a.b.C の解決を変える。JLS 6.5.2・7.1。Q87）
4. 再解析に回したファイルを解析して追記する（H 行の親型は部分型の索引に足す）。そのファイルの自分の宣言の指紋（I 行の
   3 列目）が旧キャッシュと違っていたら、または jar の変化で回したファイルなら、宣言する型を「変わった型」に加える
   （宣言に書いた型の解決先・定数の値は使う側の事実に効くので、依存を 1 段辿るだけでは足りない。jar の変化で回した
   ファイルは、jar の親の親から継承したものが指紋にも H 行にも現れないので常に加える。Q83・Q84）。
   「変わった型」が増えていれば 3 へ戻る。
   **親型の連鎖**: 型を「変わった型」に加えるときは、部分型の索引にあるその部分型も推移的に「変わった型」に加える。
   索引に関係（子 -> 親）を足すときも、親がすでに「変わった型」なら、子（とその部分型）を
   加える。どちらの向きでも広げるので、「変わった型」はいつでも索引について部分型で閉じていて、H 行を読む順・ファイルを
   解析し直す順に依らない。親の親の変化（継承したメンバー・オーバーロード・私的メンバーによる隠蔽・jar の親の版）は子の
   利用者の I 行に載らないが、これで子の利用者まで届く。何が変わったかは見ない（親のメソッドの本体・コメントだけの変更
   でも部分型の利用者を解析し直す。形の指紋で絞る作りは取りこぼしが続いたのでやめた。cache-unification-qa.md の Q77）。
   索引は差分更新のときだけ作り、型 1 つにつき文字列 1 つで持つ（子が 1 つの親は文字列、2 つ以上ならリスト）
...
（参照実装は最長 30 分。環境変数で変えられる）理由を言って失敗する。錠のファイルは放しても消さない（消した直後に別の実行が
作り直すと、2 つの実行が別々のファイルに錠をかけうる）。錠の仕組みの無いファイルシステムでは錠なしで続け、同じフォルダを
同時に使わないよう警告する。グラフを組む側も、最後まで書き終えたキャッシュか（形式の版・最終行のブロック数・最終行の後ろに
...
片方を解析するときはもう片方も解析し直す（`package-info.java`・`module-info.java` も組にする。JDT はアノテーションの付いた
パッケージ宣言に `package-info` という型を作るので、同じバッチなら 2 つ目が重複のエラーになる）。名前の違うファイルで同じ型を
宣言している場合は、ファイル名からは組にできない（どちらの宣言が読まれるか・両方が読まれるかは一緒に解析したファイルで
決まり、差分更新と全件解析とで食い違いうる。これは限界として残し、警告の文言でそう伝える）。グラフを組むときに同じ
メソッドキーが 2 つのファイルで宣言されていたら警告し、先に並ぶソースフォルダの宣言を使う（後から読んだほうを勝たせると、
キャッシュのブロックの並びで宣言の場所が変わる）。

**1 ファイルの失敗で設定ごと止めない。** JDT の再帰が深すぎる式（数千段つないだメソッド呼び出し）では
`StackOverflowError` が出る。一括パースの途中なら残りを 1 ファイルずつ解析し、溢れたファイルだけを失敗として案内する。
受け手（キャッシュへ書く側）の中で溢れた場合も、一括パースと 1 ファイルずつの解析のどちらの経路でも、そのファイルの失敗として
数える（外の catch まで抜けると、そのファイルは受け取ったとも失敗したとも数えられずに消える）。
