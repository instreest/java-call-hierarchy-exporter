...
- ビュー名は「呼び出し階層 (Exporter)」ではなく **「影響調査 (Call Hierarchy Exporter)」**にする。
...
- **ビューの名前**（§2.5 は「影響調査 (Call Hierarchy Exporter)」を推す）。Eclipse 版の「呼び出し階層 (Exporter)」と
  揃えるか、VSCode では標準機能と紛れないことを優先するか
