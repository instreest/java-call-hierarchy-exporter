...
```
Call Hierarchy Exporter: 10:31:04 時点（12 ファイル変更）  [再解析]
```
...
- コマンド名は `Call Hierarchy Exporter: …` で揃え、コマンドパレットでどちらの機能か分かるようにする
...
なお「終了行を持たないこと」自体は、この設計とは切り離して**別途 Issue で扱う**。
...
| ビューの名前 | **「影響調査 (Call Hierarchy Exporter)」**。VSCode では標準機能と紛れないことを優先する（§2.5）。Eclipse 版の名前を揃えるかは別途 Issue で検討する |
...
| `AT` の精度（メソッドの終了行） | 設計としては §4 の割り切りで進める。**終了行を持つかどうかは「行数」の問題として別途 Issue で扱う**（キャッシュの形式変更を伴うため、この設計とは切り離す） |
