...
`test/demo` では、ラムダ本体の行が囲みメソッドの下から合成メソッドの下へ1段深くなり、
`methods.csv` に合成メソッドの行が増えた。行が消えたものは無い。

`methods.csv` の `outDegree` は、ラムダを持つメソッドでは下がる
（本体の呼び出しが合成メソッドに移るため）。代わりに合成メソッドの行に出る。

### Q9. 合成メソッドを `methods.csv` に出すのはなぜか

ソース上に位置（ファイル・行）を持ち、呼び出し元と呼び出し先を持つノードだから。
出さないと `inDegree` / `outDegree` の合計が階層の行数と合わなくなり、
「どこへ消えたのか」が読み手に分からなくなる。

`role` は普通のメソッドと同じ規則で付く。生成の辺があるので `inDegree` は必ず1以上になり、
`ENTRY_CANDIDATE`（呼び出し元が無い）にはならない。
