...
- ラムダ式の本体は、javac と同じ名前（`lambda$囲みメソッド名$通し番号`）の**合成メソッド**にする。
  ...
  `methods.csv` には出さない（匿名クラスのメソッドも同様。Q9）
...
`methods.csv` は「ソース上で定義されたメソッドの一覧」だから。合成メソッドはソースに
宣言が無く、名前もこちらが付けたもの。同じ理由で、コンパイラが名前を付ける
匿名クラス（`Outer$1`）のメソッドも出さない（ローカルクラス `Outer$1Local` は
ソースに名前があるので出す）。

出さないぶん、ラムダを持つメソッドの `outDegree` は本体の呼び出しを含まない
（生成の辺1本だけを数える）。本体の中の呼び出しは `call-hierarchy.csv` で追う。

当初は「位置を持ち、辺を持つノードだから」出す設計にしたが、一覧の役割
（誰からも呼ばれていないのはどれか、を俯瞰する）から見ると合成メソッドは
必ず生成の辺から呼ばれる行でしかなく、行が増える割に判断の材料にならない。
出力対象外にした件数は実行ログに出す。
