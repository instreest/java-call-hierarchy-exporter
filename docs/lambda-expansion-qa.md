...
`methods.csv` の基準を「そのクラスのインスタンス（または型）を通じて呼び出せる定義」に
置いたから。アクセス修飾子の話ではなく、定義として呼び出せる形かどうか。
合成メソッドは名前をこちらが付けたもので、ソースからは呼び出せない。
同じ理由で static 初期化子（合成した `<clinit>`）も出さない。

匿名クラスのメソッドは出す。クラス名（`Outer$1`）はコンパイラが付けるが、
メソッド本体はソースに書かれていて、インターフェース経由でそのインスタンスから
呼び出せる定義だから。一度「コンパイラが名前を付ける型」として外したが、
基準を呼び出せるかどうかに置き直して戻した。
