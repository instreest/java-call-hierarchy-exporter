...
| `src/jche/` | 本体。`config`（設定・ビルドファイル読み取り）、`analysis`（AST 訪問・キャッシュ更新）、`graph`（呼び出しグラフ・具象クラス解決）、`report`（CSV 出力）、`cli`（対話モード）、`extension` / `builtin`（プラグイン）、`external`（jar からの被参照）、`cache`、`framework`、`util` |
