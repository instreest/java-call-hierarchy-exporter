 ...
 *                                                              （呼び出し先・フィールドを宣言した型、引数の型、親型、
 *                                                              宣言の型など。jche.analysis.BindingNames#typeNameOf）
 *                                                          (b) ソースに書かれた型の名前（SimpleType・QualifiedType・
 *                                                              NameQualifiedType の節。戻り値・throws・ローカル変数・
 *                                                              キャストの型など）
 *                                                          (c) ラムダ・メソッド参照の目標の関数型インターフェースと
 *                                                              その推移的な親
 *                                                          (d) すべての呼び出し（解決できたかどうかに関わらない）の
 *                                                              受け手の式と実引数の式の型、new する型とその外側の
 *                                                              インスタンスの式の型、super(...) の外側の式の型、
 *                                                              メソッド参照の左側の式の型
 *                                                              （jche.analysis.CallSiteRecorder#noteOperandTypes）
 *                                                          (e) 修飾された名前・フィールド参照の左側の式・名前の型
 *                                                              （{@code a.getB().count} の B。解決できなかった名前も。
 *                                                              jche.analysis.FieldAccessRecorder#noteQualifier）。
 *                                                              (d)(e) は型変数・捕捉された型変数・ワイルドカード・
 *                                                              交差型を上限の消去で数える
 *                                                          (f) 呼び出したメソッド・コンストラクタの throws の型
 *                                                              （型変数は上限の消去）
 *                                                          (g) 拡張 for 文の式・switch のセレクタ・throw の式の型
 *                                                          (h) アノテーションの型（宣言のほか、引数・ローカル変数・
 *                                                              型の使用に付いたもの）
 *                                                          (i) switch の case に書いた列挙定数の型と型パターン
 *                                                              （レコードパターンを含む）の型、とその推移的な親
 *                                                          (j) import 文の型（オンデマンド import は "pkg.*"）
 *                                                          (k) 型解決に失敗したブロック（エラーか BINDING_FAILED の
 *                                                              U 行がある）では、名前にした型（自分が宣言する型を
 *                                                              含む）すべての推移的な親
 *                                                          (f)(h)、(i) の親、(k) は {@code java.*} の型を数えない（JDK の版は
 *                                                          ヘッダ行の鍵に入っている。ただし型・メソッド・フィールドの宣言に
 *                                                          付いたアノテーションは、{@code java.lang} 以外のものを H・D・V 行に
 *                                                          書くので (a) として載る）。
 *                                                          <b>型の形の指紋</b>は、このファイルが宣言する型（入れ子・匿名・
 *                                                          ローカルを含む）ごとの行（jche.analysis.TypeShape）を並べ替えて
 *                                                          取ったハッシュの頭 16 文字。宣言する型が無ければ空。行は、型の
 *                                                          キー・修飾子・種類、推移的な親型、型引数とその上限、自分と
 *                                                          親型（{@code java.*} の型は除く）が宣言するメソッド（キー・
 *                                                          修飾子・戻り値と引数の型・可変長引数か・throws の型と検査例外か）・
 *                                                          フィールド・入れ子の型。私的メンバーは、それを宣言した型の親型に
 *                                                          私的でない同じ名前のメンバーがあるときだけ入れ、私的な
 *                                                          コンストラクタは入れない。解析し直した結果これが前回と
 *                                                          違えば、このファイルの型を使う側も解析し直す（親型の連鎖）。
 ...
 *       ファイルも解析し直す（「定数の連鎖」）</li>
 *   <li>親の親の変化（継承したメンバー・親型）は子の利用者の I 行に載らない。解析し直したファイルの型の形
 *       （I 行の 2 列目）が前回と違えば、その型を参照するファイルも解析し直す（「親型の連鎖」）</li>
 *   <li>前回は無かった型・見えなかった型は、参照していた側の I 行に載らない。型解決に失敗していたブロックのうち、
 *       解決できなかった名前（I 行の 3 列目）が変わった型に当たるものと、同じパッケージに足した型に名前を
