 ...
 *
 * <h2>なぜ文字列ではなく表か</h2>
 * 読み手は以前、値グラフを出所の文字列（{@link Origin}）に組み直したものを受け取っていた
 * （{@code docs/cache-unification-qa.md} の「読み手が値の表を読む」）。文字列の文法は {@code | ; { }} を
 * 区切りに使うので、値そのもの（文字列リテラル・定数）がそれらの文字を含むと、読み手が途中で切って
 * 別の値に読み違えていた。
 ...
 * （{@link StringPool} の番号）と、項目の並びを持つ。項目は次の順に並ぶ（以前の出所の文字列の並びと同じ）。
 * <pre>
