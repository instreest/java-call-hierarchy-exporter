...
final class MessagesEn {
    ...
    private static String[] exporter() {
        return new String[] {
            ...
            "exporter.warnings.config.remedy2", "Relative paths start from different places: source.folders / library.folders / external.library.folders from project.root, and the other items (project.root / library.jars / output.folder / cache.folder, etc.) from the config file's folder.",
            ...
            "exporter.warnings.incomplete.remedy2", "If the row limit (max.rows) was reached, raise it or narrow the entry points with entry.packages. The same limit also applies to the constructor calls and excluded methods the walk passes in a row without writing a row.",
    ...
    private static String[] config() {
        return new String[] {
            ...
            "config.plugin.readFailed", "Cannot read the compiled classes of the extension. Continuing without it: {0} ({1})",
            "config.plugin.notDeleted", "Could not delete the compile output of the extension: {0} ({1})",
    ...
    private static String[] analysis() {
        return new String[] {
            ...
            "analysis.tooDeep", "the expression is nested too deeply for the Java parser (stack overflow; for example thousands of chained method calls). The calls in this file are not in the output. Split the expression, or run the analysis with a larger thread stack (for example JDK_JAVA_OPTIONS=-Xss16m)",
            "analysis.hashFailed", "Cannot hash the source (this file is analyzed again every time): {0} ({1})",
            "analysis.cache.incompatible", "[cache] The format, source level, encoding, JDK, JDT or source folder order (or nesting) differs, so the existing cache is discarded",
            "analysis.cache.foldersChanged", "[cache] Source folders were added or removed (the order of the others is unchanged), so the cache is kept: files in added folders are analyzed as added files, and files in removed folders are dropped as deleted files",
            "analysis.cache.environmentRejected", "[cache] The Java parser does not accept the class path built from the source folders and dependency jars of this run ({0}), so no file can be analyzed. The cache is not used, so the result is the same as a full analysis",
            "analysis.cache.headDamaged", "[cache] The first lines of the existing cache (dependency jars and source list) failed the integrity check, so it is discarded and everything is analyzed",
            ...
            "analysis.resume.incompatible", "[cache] The interrupted run differs in format, source level, encoding, JDK, JDT or source folder order, so nothing is carried over",
            "analysis.resume.headDamaged", "[cache] The first lines of the temporary file of the interrupted run failed the integrity check, so nothing is carried over",
            ...
            "analysis.batchFailed", "The Java parser failed in a batch before returning {0} ({1})",
            "analysis.batchTooDeep", "The Java parser ran out of stack in a batch before returning {0} (only a file that is too deep fails)",
            "analysis.batchStopped", "The Java parser stopped in a batch without an error before returning {0}",
            "analysis.batchFailedEarly", "The Java parser failed in a batch before any file was finished ({0} file(s)), so it is not known which file caused it ({1})",
            "analysis.batchTooDeepEarly", "The Java parser ran out of stack in a batch before any file was finished ({0} file(s)), so it is not known which file is too deep (only a file that is too deep fails)",
    ...
    private static String[] graph() {
        return new String[] {
            ...
            "graph.cacheIncomplete", "Internal error while building the graph: the cache {0} is not a finished cache of this version ({1} file block(s), last line says {2}). Nothing is written, so that no calls are silently missing. Run again",
            "graph.duplicateType", "The type {0} is declared in both {1} and {2}. Which declaration the Java parser reads depends on which files it analyzes together, so the calls in one of them may be missing from the output, or both may be merged into one type (methods with the same signature are combined and attributed to {3}), and the result can differ between runs (for example between a full and an incremental analysis). Remove or rename one of the two declarations. If the two files are in different source folders (for example a copy of the same class kept in another folder), listing only one of those folders in source.folders also fixes it",
            "graph.duplicateType.more", "More types are declared in two files (only the first {0} pairs are listed)",
    ...
    private static String[] report() {
        return new String[] {
            ...
            "report.walker.maxSilentNodes", "The walk passed {0} constructor calls and excluded methods (exclude.packages) in a row without writing a row, so the output stops here. This uses the same limit as max.rows",
    ...
    private static String[] cache() {
        return new String[] {
            ...
            "cache.lock.waiting", "[cache] Another run is using the cache folder {0}; waiting until it finishes (at most {1} seconds)",
            "cache.lock.acquired", "[cache] The other run finished; continuing",
            "cache.lock.unsupported", "Cannot lock the cache folder {0} (the file system does not support file locks: {1}). Continuing without the lock: do not run two analyses on this cache folder at the same time, or set cache.folder to a local folder",
            "cache.lock.timeout", "Another run kept using the cache folder {0} for more than {1} seconds, so this run stops. Wait for the other run (a CLI run, the Eclipse or VS Code plugin, or a CI job on the same folder) to finish and run again. The wait can be changed with the environment variable {2} (seconds)",
            "cache.lock.interrupted", "Interrupted while waiting for another run to finish using the cache folder {0}",
            "cache.lock.badWait", "[cache] Ignoring {0}={1} (not a number of seconds); using the default",
