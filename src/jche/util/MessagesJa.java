...
final class MessagesJa {
    ...
    private static String[] analysis() {
        return new String[] {
            ...
            "analysis.batchFailed", "一括解析で、Java のパーサが {0} を返す前に失敗しました（{2}）。このファイルは脇に置いて同じフォルダのファイル・import しているファイルと一緒に解析し直し、バッチの残り {1} 件はまとめて続けます",
            "analysis.batchTooDeep", "一括解析で、Java のパーサが {0} を返す前にスタックが溢れました。このファイルは脇に置いて同じフォルダのファイル・import しているファイルと一緒に解析し直し（入れ子の深すぎるファイルだけが失敗になります）、バッチの残り {1} 件はまとめて続けます",
            "analysis.batchStopped", "一括解析で、Java のパーサが {0} を返す前にエラーなしで止まりました。このファイルは脇に置いて同じフォルダのファイル・import しているファイルと一緒に解析し直し、バッチの残り {1} 件はまとめて続けます",
