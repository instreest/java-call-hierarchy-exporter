...
final class MessagesJa {
    ...
    private static String[] graph() {
        return new String[] {
            ...
            "graph.duplicateType", "型 {0} が {1} と {2} の両方で宣言されています。Java のパーサがどちらの宣言を読むかは一緒に解析するファイルの組み合わせで決まるので、片方の呼び出しが出力に出ないことも、両方が 1 つの型にまとめられる（同じシグネチャのメソッドは 1 つにして {3} の宣言として出す）こともあり、実行によって（たとえば全件解析と差分更新とで）結果が変わりえます。重複を消すか、source.folders にはどちらかのフォルダだけを書いてください",
