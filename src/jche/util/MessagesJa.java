...
final class MessagesJa {
    ...
    private static String[] analysis() {
        return new String[] {
            ...
            "analysis.cache.incompatible", "[cache] 形式・ソースレベル・文字コード・JDK・JDT・ソースフォルダの並びのいずれかが異なるため既存キャッシュを破棄します",
    ...
    private static String[] graph() {
        return new String[] {
            ...
            "graph.duplicateType", "型 {0} が {1} と {2} の両方で宣言されています（同じクラスが 2 つのソースフォルダにある）。解析できるのは片方だけで、もう片方の呼び出しは出力に出ません。メソッドの宣言は {3} のものを使います。重複を消すか、source.folders にはどちらかのフォルダだけを書いてください",
