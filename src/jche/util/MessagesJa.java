...
final class MessagesJa {
    ...
    private static String[] exporter() {
        return new String[] {
            ...
            "exporter.warnings.config.remedy2", "相対パスの起点は項目ごとに違います。source.folders / library.folders / external.library.folders は project.root から、それ以外（project.root / library.jars / output.folder / cache.folder など）は設定ファイルのフォルダからです。",
            ...
            "exporter.warnings.incomplete.remedy2", "行数の上限（max.rows）に達した場合は、上限を上げるか、entry.packages で起点を絞ってください。行を書かないまま続けて通るコンストラクタ呼び出しと除外メソッドの数にも同じ上限が掛かります。",
    ...
    private static String[] config() {
        return new String[] {
            ...
            "config.plugin.readFailed", "コンパイルした拡張のクラスを読めません。拡張なしで続行します: {0} ({1})",
            "config.plugin.notDeleted", "拡張のコンパイル先を消せませんでした: {0} ({1})",
    ...
    private static String[] analysis() {
        return new String[] {
            ...
            "analysis.batchFailed", "一括解析が {0} を読んでいるときに失敗したため、そのファイルは単独で、ほかの {1} 件はもう一度まとめて解析します ({2})",
            "analysis.batchTooDeep", "一括解析で {0} を読んでいるときに Java のパーサのスタックが溢れたため、そのファイルは単独で、ほかの {1} 件はもう一度まとめて解析します（入れ子の深すぎるファイルだけが失敗になります）",
            "analysis.batchFailedEarly", "{0} 件の一括解析が、どのファイルも解析し終わらないうちに失敗し、どのファイルが原因か分からないため、半分ずつに分けてまとめて解析し直します ({1})",
            "analysis.batchTooDeepEarly", "{0} 件の一括解析で、どのファイルも解析し終わらないうちに Java のパーサのスタックが溢れたため、半分ずつに分けてまとめて解析し直します（入れ子の深すぎるファイルだけが失敗になります）",
    ...
    private static String[] report() {
        return new String[] {
            ...
            "report.walker.maxSilentNodes", "行を書かないまま、行にならないコンストラクタ呼び出しと除外メソッド（exclude.packages）を続けて{0}件通ったため打ち切りました。上限は max.rows と同じ値です",
