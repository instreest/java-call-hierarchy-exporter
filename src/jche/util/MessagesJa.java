...
final class MessagesJa {
    ...
    private static String[] analysis() {
        return new String[] {
            ...
            "analysis.batchRetry", "{1} を含む残り {0} 件を、{1} と同じフォルダのファイル・{1} が名前を書いた型（import と完全修飾名）のファイルを添えて、まとめて解析し直します",
            "analysis.batchSetAside", "関わるファイルを添えても Java のパーサが {0} で止まるので、このファイルは脇に置いて 1 つだけで解析し直し、バッチの残り {1} 件はまとめて続けます",
