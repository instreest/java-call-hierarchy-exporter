...
final class MessagesJa {
    ...
    private static String[] exporter() {
        return new String[] {
            ...
            "exporter.warnings.incomplete.remedy2", "行数の上限（max.rows）に達した場合は、上限を上げるか、entry.packages で起点を絞ってください。行にならずに通るコンストラクタ呼び出しと除外メソッドの数にも同じ上限が掛かります。",
    ...
    private static String[] report() {
        return new String[] {
            ...
            "report.walker.maxSilentNodes", "行にならないコンストラクタ呼び出しと除外メソッド（exclude.packages）を{0}件通ったため打ち切りました。上限は max.rows と同じ値です",
