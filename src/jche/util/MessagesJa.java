...
final class MessagesJa {
    ...
    private static String[] analysis() {
        return new String[] {
            ...
            "analysis.batchFailed", "一括解析で、Java のパーサが {0} を返す前に失敗しました（{1}）",
            "analysis.batchTooDeep", "一括解析で、Java のパーサが {0} を返す前にスタックが溢れました（入れ子の深すぎるファイルだけが失敗になります）",
            "analysis.batchStopped", "一括解析で、Java のパーサが {0} を返す前にエラーなしで止まりました",
            "analysis.batchFailedEarly", "{0} 件の一括解析で、Java のパーサがどのファイルも解析し終わらないうちに失敗したため、どのファイルが原因か分かりません（{1}）",
            "analysis.batchTooDeepEarly", "{0} 件の一括解析で、Java のパーサがどのファイルも解析し終わらないうちにスタックが溢れたため、どのファイルが深すぎるのか分かりません（入れ子の深すぎるファイルだけが失敗になります）",
